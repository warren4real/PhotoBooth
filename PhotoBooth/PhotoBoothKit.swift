...
enum PhotoFilter: String, CaseIterable, Identifiable, Hashable {
    ...
    func apply(to image: UIImage) -> UIImage {
        guard self != .none, let ciImage = CIImage(image: image) else {
            return image
        }
        ...
        guard let output = filter.outputImage,
              let cgImage = PhotoFilter.context.createCGImage(
                output,
                from: ciImage.extent
              ) else {
            return image
...
enum PhotoStripComposer {
    ...
    static func compose(
        ...
        let totalHeight =
            (photoHeight * CGFloat(filtered.count))
        ...
        return renderer.image { ctx in
            ...
            let titleSize = captionText.size(
            ...
            let titleRect = CGRect(
                ...
                y: totalHeight
                    - footerHeight
                    + (footerHeight - titleSize.height) / 2
                    - 10,
        ...
        PHPhotoLibrary.requestAuthorization(
            ...
            default:
                ...
                DispatchQueue.main.async {
                    completion(
                        false,
                        "Photo library access is disabled. Enable it in Settings to save photos."
                    )
                }
            }
        }
